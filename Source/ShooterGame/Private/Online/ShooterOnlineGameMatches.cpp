...
{
	// UE5: IOnlineGameMatches::ProvideGameMatchFeedback was removed from the online subsystem
	...
	if (OwnerId.IsValid())
	{
		if (IsClientRepresentative(OwnerId))
		{
			...
			for (TActorIterator<AShooterPlayerState> It(WeakGameInstance->GetWorld()); It; ++It)
			{
				...
				// UE5: FUniqueNetIdString constructor changed - use Create() factory method
				TSharedPtr<const FUniqueNetId> PlayerNetId = bIsABot ? FUniqueNetIdString::Create(FString::FromInt((*It)->GetPlayerId()), NAME_None) : PlayerId;
