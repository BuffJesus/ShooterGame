...
void InitClassReplicationInfo(FClassReplicationInfo& Info, UClass* Class, bool bSpatialize, float ServerMaxTickRate)
{
	...
	Info.ReplicationPeriodFrame = FMath::Max<uint32>( (uint32)FMath::RoundToFloat(ServerMaxTickRate / CDO->GetNetUpdateFrequency()), 1);
	...
	UE_LOG(LogShooterReplicationGraph, Log, TEXT("Setting replication period for %s (%s) to %d frames (%.2f)"), *Class->GetName(), *NativeClass->GetName(), Info.ReplicationPeriodFrame, CDO->GetNetUpdateFrequency());
}
...
void UShooterReplicationGraph::InitGlobalActorClassSettings()
{
	...
	for (UClass* ReplicatedClass : AllReplicatedClasses)
	{
		...
		InitClassReplicationInfo(ClassInfo, ReplicatedClass, bClassIsSpatialized, NetDriver->GetNetServerMaxTickRate());
...
void UShooterReplicationGraph::InitGlobalGraphNodes()
{
	...
	{
		// UE5: AddSpatialRebuildBlacklistClass may have been removed or renamed
		// TODO: Verify if this API still exists in UE5.7 and update accordingly
